...
const FilterNode = () => {
  ...
  useEffect(() => {
    ...
    if (first) {
      ...
      const type = first.type as keyof typeof FindConditions;
      if (type) {
        const condition = FindConditions[type];
    ...
    if (inputRef.current && selectedColumn && selectedCondition) {
      ...
      if (nodeId) {
        ...
        if (id) {
          ...
          const type: 'number' | 'string' = dataType?.type as 'number' | 'string';
      ...
      {columnNames?.length > 0 && (
        <>
          <SelectDropdown
            ...
            className="nodrag"
          />
          {selectedCondition && <InputBox className="nodrag" ref={inputRef} label="Search term" />}
        </>
      )}
    </CustomNode>
  );
